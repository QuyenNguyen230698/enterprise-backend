...
const BASE = `${PYTHON_URL}/api/v1/asset-handover`;
...
const assetHandoverService = {
  ...
  async updateAssets(_reqUser, id, assets = []) {
    const res = await axios.put(`${BASE}/${id}/assets`, { assets });
    return res.data?.data || null;
  },
  ...
  async getByOffboarding(_reqUser, offboardingId) {
    const res = await axios.get(`${BASE}/by-offboarding/${offboardingId}`);
    return res.data?.data ?? null;
  },
};

module.exports = assetHandoverService;
